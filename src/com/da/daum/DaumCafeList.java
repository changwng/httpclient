...
public class DaumCafeList {
		...
		try {
			...
			for (int i = init_page; i < max_page; i++) {
				...
				for (DaumListVo vo : lst) {
					...
					System.out.println("=========================================");
					file = SAVE_DIR+"/user/"+vo.getIdAlais().replaceAll("\\*", "").replaceAll("\\/", "_")+".txt";
	...
	private List<DaumListVo> getBoardList(HttpClient httpclient,   String n_page ) throws ClientProtocolException, IOException
		...
		String strUrl =   host_url+"/CHILIL/LAK?prev_page="+String.valueOf(prev_page)+"&firstbbsdepth=000BH&lastbbsdepth=000Aw&noticeYn=&page=" + n_page;
		if(!"1".equals(n_page))
		{
